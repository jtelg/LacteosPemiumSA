...
const APIConsultas = {
  ...
  usuario: {
    ...
    GET_XID: async (iduser, local) => {
      let baseUrl = process.env.NEXTAUTH_URL;
      if (local) baseUrl = '';
      const url = `${baseUrl}/api/user/${iduser}?path=USUARIO_GET_XID`;
      const re = await fetch(url);
      if (re.ok) return (await re.json())[0];
      return false;
    },
    UPDATE: async (iduser, campo, valor) => {
      ...
      const url = `/api/user/${iduser}?path=USUARIO_UPDATE`;
      const updres = await fetch(url, options);
      if (updres.ok) {
        return true;
      }
    ...
    GlobalInfoAdmin: async (dispatch, local) => {
      ...
      // });
      // fetch(`${baseUrl}/botwpp`).then((res) => {});
      // const channel = pusherInitFront.subscribe('ventas_chanel');
      // channel.bind('VENTA_RELOAD', function (data) {
      //   dispatch(VENTAS_SET(data));
      //   dispatch(RELOAD_CAJA(true));
      // });
      // await APIConsultas.ventas.VENTA_RELOAD(true);
      // const resp = {
      //   botWPP: '',
      //   pusher: ''
      // };
  ...
  ventas: {
    ...
    GET_ALL: async (idcaja, local, bnd) => {
      let baseUrl = process.env.NEXTAUTH_URL;
      if (local) baseUrl = '';
    ...
    GET_PRODS_XID: async (idventa, local) => {
      let baseUrl = process.env.NEXTAUTH_URL;
      if (local) baseUrl = '';
      const url = `${baseUrl}/api/venta?path=VENTA_GET_PROD_XID&id=${idventa}`;
      const resprod = await fetch(url);
      let dataprod = null;
      if (resprod.ok) {
        dataprod = await resprod.json();
  ...
  caja: {
    ...
    GET_TODO: async (local) => {
      let baseUrl = process.env.NEXTAUTH_URL;
      if (local) baseUrl = '';
      const repscateg = await fetch(`${baseUrl}/api/caja/?path=CAJA_GET_TODO`);
