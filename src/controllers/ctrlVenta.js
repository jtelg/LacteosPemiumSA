...
const ctrlVenta = {
  VENTA_GET_LIST: async (req, res, resolve) => {
    ...
    const sqlApply = 'CALL SP_ventas_get_tablero()';
  ...
  VENTA_ADD: async (req, res, resolve) => {
    ...
    try {
      ...
      ctrlVenta.ARTXVENTA_ADD(req, res, resolve, idventa);
    } catch (error) {
      console.error(error);
      res.status(500).end();
      return resolve();
  ...
  ARTXVENTA_ADD: async (req, res, resolve, idventa) => {
    let string = '';
    ...
    try {
      ...
      ctrlVenta.SEGUIMIENTO_ADD(req, res, resolve, idventa);
    } catch (error) {
      console.error(error);
      res.status(500).end();
      return resolve();
  ...
  SEGUIMIENTO_ADD: async (_, res, resolve, idventa) => {
    const sqlS = `INSERT INTO seguimiento (idventa, idestado, create_time)
                      VALUES (${idventa}, 1, NOW());
                      `;
    try {
      await conexionDB.query(sqlS);
      res.end();
