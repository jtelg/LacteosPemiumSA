...
const ctrlProducto = {
  PRODUCTO_ADD: async (req, res, resolve) => {
    ...
    try {
      ...
      // const rewo = await addWooCommerce(req.body);
  ...
  PRODUCTO_GET_LIST: async (req, res, resolve) => {
    ...
    const P_visible = req.query.visible ? `(${req.query.visible})` : '(0,1)';
    let sql_idcateg = '';
    let sql_idsubc = '';
    if (P_idcateg !== '0') {
  ...
  COLORXPROD_DELETE: async (req, res, resolve) => {
    ...
    try {
      ...
      if (color.arrimages?.length > 0) {
        const re1 = await APIConsultas.color.DEL_IMAGE_PHP_TODO(color);
        if (re1.data.success) {
